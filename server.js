...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 25.0, calls_per_day: 50, label: 'Starter' },
  pro:        { price_usd: 99.0, calls_per_day: 500, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[insurance] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      ...
      x402: {
        type: 'x402', version: '1', kind: 'subscription_insurance',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Insurance', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Insurance match-and-attest — Hive matches insurer to insured. Partners: Nexus Mutual, Etherisc',
    brand: '#C08D23',
